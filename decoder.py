...
class Scorer(object):
    ...
    def evaluate(self, sentence, bos=True, eos=False):
        lm = self._alpha*self.language_model_score(sentence, bos, eos)
        word_cnt = self._beta*self.word_count(sentence)
        return (lm, word_cnt)
...
def ctc_beam_search_decoder(probs_seq,
    ...
    for (seq, prob) in prefix_set_prev.items():
        if prob > 0.0:
            ...
            lm_score, word_count = ext_scoring_func(result, True, True)
            log_prob = np.log(prob)
            total_score = log_prob+lm_score+word_count
            beam_result.append([total_score, log_prob, lm_score, word_count, result])
